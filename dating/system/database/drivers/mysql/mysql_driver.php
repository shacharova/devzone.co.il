...
class CI_DB_mysql_driver extends CI_DB {
    ...
    function db_set_charset($charset, $collation) {
        if (!isset($this->use_set_names)) {
            ...
            $this->use_set_names = (version_compare(PHP_VERSION, '5.2.3', '>=') && version_compare(mysql_get_server_info(),
                                                                                                   '5.0.7',
                                                                                                   '>=')) ? FALSE : TRUE;
        }
    ...
    function _list_columns($table = '') {
        return "SHOW COLUMNS FROM " . $this->_protect_identifiers($table, TRUE,
                                                                  NULL, FALSE);
    }
    ...
    function _escape_identifiers($item) {
        ...
        foreach ($this->_reserved_identifiers as $id) {
            if (strpos($item, '.' . $id) !== FALSE) {
                $str = $this->_escape_char . str_replace('.',
                                                         $this->_escape_char . '.',
                                                         $item);
    ...
    function _update($table, $values, $where, $orderby = array(), $limit = FALSE) {
        ...
        $orderby = (count($orderby) >= 1) ? ' ORDER BY ' . implode(", ",
                                                                   $orderby) : '';

        $sql = "UPDATE " . $table . " SET " . implode(', ', $valstr);

        $sql .= ($where != '' AND count($where) >= 1) ? " WHERE " . implode(" ",
                                                                            $where) : '';
